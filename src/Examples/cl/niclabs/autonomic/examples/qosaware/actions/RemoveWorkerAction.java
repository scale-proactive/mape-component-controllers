...
public class RemoveWorkerAction extends Action {
	...
	public Object execute(Component solver, PAGCMTypeFactory typeFactory, PAGenericFactory genericFactory) {
		...
		try {			
			Component[] subComps = Action.getSubComponent(solver, "Jonas2");
			if (subComps.length > 0) {
		...
		try {
			...
			Utils.getPABindingController(dispatcher).unbindFc("jeeReqm");			
			Utils.getPABindingController(worker).unbindFc("jeeReturn");			
